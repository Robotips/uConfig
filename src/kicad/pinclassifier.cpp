...
PinClassifier::PinClassifier(const Component &component)
    : _component(component)
{
    ...
    for (int i=1; i<pins.count(); i++)
    {
        ...
        if(compare==0 || (compare<=2 && lastCompare>=2)) categ.last().append(PinCompare(pins[i].name(),true));
        else categ.append(QList<PinCompare>()<<PinCompare(pins[i].name(),true));
        lastCompare = compare;
    }
...
PinCompare::PinCompare(const QString &name, bool number)
{
    ...
    {
        if(_internalName.contains(QRegExp("([0-9][0-9])"))) _internalName.replace(QRegExp("([0-9][0-9])"),"0\\1");
        else _internalName.replace(QRegExp("([0-9])"),"00\\1");
    }
}
...
int PinCompare::distanceTo(const PinCompare &other)
{
    ...
    {
        if(_internalName.at(i)!=other._internalName.at(i)) dist++;
    }
    return dist;
}
